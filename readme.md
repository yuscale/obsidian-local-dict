...
以下设置用于控制词典内容在“简略视图”下的展示行为，可通过设置选择器来隐藏不必要的部分，并在隐藏的部分中保留特定元素。

| 设置项 | 说明 |
|--------|------|
| **全局隐藏元素选择器**| 所有模式下始终隐藏的 HTML 元素。支持多个 CSS 选择器，每行一个。例如：|
| **简略模式下隐藏选择器** | 仅在“简略视图”启用时隐藏的元素。可以隐藏一些次要内容、例句、标签等。 |
| **简略模式保留的隐藏元素子选择器** | 即使父元素在“简略模式下隐藏”，仍可从中保留其部分子元素。每行一条规则，格式为：`父选择器 > 子选择器`。
...
```html
<div class="example">This is an example sentence.</div>
...
效果：在“简略视图”下，`.note` 整体会被隐藏，但其中的 `<strong>` 元素会被显示。

---

📖 **提示**：

- 保留规则中 `>` 为 **必须**，代表从哪个隐藏元素中恢复显示子元素。
- 若设置失效，请检查是否有拼写错误或选择器不正确。
- 所有选择器基于 CSS 标准。
